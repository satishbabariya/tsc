...
# Simple Memory Validation Test Runner
# This script runs tests with memory sanitizers using environment variables

set -e  # Exit on any error

echo "🧠 Running Simple Memory Validation Tests"
echo "=========================================="
...
run_memory_test() {
    ...
    if env $env_vars "$BUILD_DIR/tsc" "$TEST_DIR/$test_file" 2>"$OUTPUT_DIR/${test_name}_${sanitizer_type}_compile.log" >"$OUTPUT_DIR/${test_name}_${sanitizer_type}_compile.out"; then
        echo -e "  ${GREEN}✅ Compilation successful with ${sanitizer_type}${NC}"
        
        # Check for memory errors in the output
        if grep -q "ERROR: AddressSanitizer\|ERROR: MemorySanitizer\|ERROR: UndefinedBehaviorSanitizer\|AddressSanitizer: detected memory\|MemorySanitizer: detected memory" "$OUTPUT_DIR/${test_name}_${sanitizer_type}_compile.log"; then
            echo -e "  ${RED}❌ Memory error detected by ${sanitizer_type}${NC}"
            echo "  📋 Memory error details:"
            grep -A 5 -B 5 "ERROR:\|AddressSanitizer:\|MemorySanitizer:" "$OUTPUT_DIR/${test_name}_${sanitizer_type}_compile.log" | head -20
...
# Test with AddressSanitizer (using existing build)
...
# Test with MemorySanitizer (using existing build)
echo -e "${BLUE}🔍 Testing with MemorySanitizer${NC}"
echo "=================================="

for test_file in "${MEMORY_TEST_FILES[@]}"; do
    run_memory_test "$test_file" "MemorySanitizer"
done
