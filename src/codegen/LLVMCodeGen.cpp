...
bool LLVMCodeGen::generateCode(Module& module, SymbolTable& symbolTable, 
    ...
    // TSC_LOG_DEBUG("LLVMCodeGen received SymbolTable at address:" + std::string(typeid(symbolTable_).name()) , "CodeGen");
...
void LLVMCodeGen::visit(NewExpression& node) {
    ...
    // TSC_LOG_DEBUG("Current insert point at start of NewExpression:" + std::string(typeid(builder_->GetInsertBlock()).name()) , "CodeGen");
    ...
    if (auto identifier = dynamic_cast<Identifier*>(node.getConstructor())) {
        ...
        if (classSymbol) {
            // TSC_LOG_DEBUG("Class symbol kind:"  + std::to_string(static_cast<int>(classSymbol->getKind())) , "CodeGen");
        ...
        // TSC_LOG_DEBUG("Class type:" + classType->toString() + ", kind: "  + std::to_string(static_cast<int>(classType->getKind())) , "CodeGen");
        ...
        if (node.hasExplicitTypeArguments()) {
            ...
            // TSC_LOG_DEBUG("Number of type arguments:"  + std::to_string(typeArguments.size()) , "CodeGen");
            
            // Create a GenericType for this instantiation
            auto genericType = typeSystem_->createGenericType(classType, typeArguments);
            // TSC_LOG_DEBUG("Created GenericType:" + genericType->toString() + ", kind: "  + std::to_string(static_cast<int>(genericType->getKind())) , "CodeGen");
...
void LLVMCodeGen::visit(CallExpression& node) {
    ...
    if (auto identifier = dynamic_cast<Identifier*>(node.getCallee())) {
        ...
        if (!function) {
            ...
            if (symbolValue && llvm::isa<llvm::Function>(symbolValue)) {
                ...
                if (!symbol) {
                    ...
                    // TSC_LOG_DEBUG("Current scope:"  + std::string(typeid(symbolTable_->getCurrentScope()).name()) ", Global scope: " + symbolTable_->getGlobalScope() , "CodeGen");
                    if (symbol) {
        ...
        if (calleeValue && llvm::isa<llvm::Function>(calleeValue)) {
            ...
            // TSC_LOG_DEBUG("CallExpression - Processing method call with" + std::to_string(node.getArguments().size()) + " arguments" , "CodeGen");
...
void LLVMCodeGen::visit(ArrayLiteral& node) {
    // TSC_LOG_DEBUG("ArrayLiteral visitor called with" + std::to_string(node.getElements().size()) + " elements" , "CodeGen");
    ...
    if (node.getProperty() == "length") {
        ...
        if (auto* identifier = dynamic_cast<Identifier*>(node.getObject())) {
            ...
            if (symbol) {
                ...
                // TSC_LOG_DEBUG("PropertyAccess - symbol type kind:"  + std::to_string(static_cast<int>((symbol->getType())->getKind())) , "CodeGen");
        ...
        if (auto identifier = dynamic_cast<Identifier*>(node.getObject())) {
            ...
            if (symbol) {
                ...
                if (symbol->getType()->getKind() == TypeKind::Generic) {
                    ...
                    // TSC_LOG_DEBUG("Symbol type is not Generic, it's:"  + std::to_string(static_cast<int>((symbol->getType())->getKind())) , "CodeGen");
                }
            } else {
...
void LLVMCodeGen::visit(BlockStatement& node) {
    ...
    // TSC_LOG_DEBUG("LLVMCodeGen processing block with current scope:"  + std::string(typeid(symbolTable_->getCurrentScope()).name()) , "CodeGen");
    ...
    // TSC_LOG_DEBUG("LLVMCodeGen finished processing block with current scope:"  + std::string(typeid(symbolTable_->getCurrentScope()).name()) , "CodeGen");
...
void LLVMCodeGen::generateMonomorphizedMethods(const GenericType& genericType, Symbol* classSymbol) {
    ...
    // TSC_LOG_DEBUG("Found class declaration with" + std::to_string(classDecl->getMethods().size()) + " methods" , "CodeGen");
...
void LLVMCodeGen::generateMonomorphizedMethod(const MethodDeclaration& method, const GenericType& genericType, const String& mangledName) {
    ...
    for (const auto& param : method.getParameters()) {
        ...
        if (param.type) {
            // TSC_LOG_DEBUG("Parameter type kind:"  + std::to_string(static_cast<int>(param.type->getKind())) , "CodeGen");
...
void LLVMCodeGen::generateFunctionBody(llvm::Function* function, const FunctionDeclaration& funcDecl) {
    ...
    if (functionScope) {
        ...
        // TSC_LOG_DEBUG("Current scope after navigation:"  + std::string(typeid(symbolTable_->getCurrentScope()).name()) " (type: "  + std::string(typeid(static_cast<int>(symbolTable_->getCurrentScope()).name())->getType()) + ", name: "  + std::string(typeid(symbolTable_->getCurrentScope()).name())->getName() + ")" , "CodeGen");
    ...
    if (funcDecl.getName() == "main") {
        ...
        if (deepestScope && deepestScope != functionScope) {
            // TSC_LOG_DEBUG("Navigating to deepest child scope:" + deepestScope + " (type: "  + std::to_string(static_cast<int>(deepestScope->getType())) + ")" , "CodeGen");
            symbolTable_->navigateToScope(deepestScope);
        }
    } else {
        // TSC_LOG_DEBUG("LLVMCodeGen could not find function scope:" + funcDecl.getName() + ", using current scope: "  + std::string(typeid(symbolTable_->getCurrentScope()).name()) , "CodeGen");
    }
    ...
    // TSC_LOG_DEBUG("LLVMCodeGen finished processing function:" + funcDecl.getName() + " with current scope: "  + std::string(typeid(symbolTable_->getCurrentScope()).name()) , "CodeGen");
}
    ...
    for (size_t i = 0; i < capturedVariables.size(); ++i) {
        ...
        if (varValue) {
            ...
            if (varValue->getType()->isPointerTy()) {
                ...
                // TSC_LOG_DEBUG("Symbol type kind:"  + std::to_string(static_cast<int>((symbol->getType())->getKind())) , "CodeGen");
...
void LLVMCodeGen::visit(ArrayDestructuringPattern& node) {
    // TSC_LOG_DEBUG("ArrayDestructuringPattern visitor called with" + std::to_string(node.getElements().size()) + " elements" , "CodeGen");
...
void LLVMCodeGen::visit(ObjectDestructuringPattern& node) {
    // TSC_LOG_DEBUG("ObjectDestructuringPattern visitor called with" + std::to_string(node.getProperties().size()) + " properties" , "CodeGen");
