...
namespace tsc {
    ...
    shared_ptr<Type> GenericConstraintChecker::substituteTypeParameters(shared_ptr<Type> type,
                                                                        const std::unordered_map<String, shared_ptr<
                                                                            Type> > &substitutions) {
        if (!type) {
            return type;
        }
    ...
    bool GenericConstraintChecker::checkPrimitiveConstraint(shared_ptr<Type> constraint,
                                                            shared_ptr<Type> typeArgument) {
        if (!constraint || !typeArgument) {
            return true;
        }
    ...
    bool GenericConstraintChecker::checkInterfaceConstraint(shared_ptr<Type> constraint,
                                                            shared_ptr<Type> typeArgument) {
        if (!constraint || !typeArgument) {
            return true;
        }
    ...
    bool GenericConstraintChecker::checkIntersectionConstraint(shared_ptr<Type> constraint,
                                                               shared_ptr<Type> typeArgument) {
        if (!constraint || !typeArgument) {
            return true;
        }
    ...
    shared_ptr<Type> GenericConstraintChecker::substituteInType(shared_ptr<Type> type,
                                                                const std::unordered_map<String, shared_ptr<Type> > &
                                                                substitutions) {
        if (!type) {
            return type;
        }
