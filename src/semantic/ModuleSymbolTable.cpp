...
bool ModuleSymbolManager::bindExportsToImports() {
    try {
        ...
        TSC_LOG_DEBUG("Total modules to process:" + moduleTables_.size() , "Semantic");
    ...
    for (const auto& pair : moduleTables_) {
        ...
        TSC_LOG_DEBUG("Module has" + moduleTable->getImportedSymbols().size() + " imported symbols" , "Semantic");
        ...
        for (const auto& imported : moduleTable->getImportedSymbols()) {
            ...
            if (!exportedSymbol) {
                ...
                TSC_LOG_DEBUG("Source module has" + sourceModule->getExportedSymbols().size() + " exported symbols" , "Semantic");
    ...
    return success;
        ...
        TSC_LOG_DEBUG("Exception in bindExportsToImports:" + e.what() , "Semantic");
