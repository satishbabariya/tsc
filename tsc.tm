# TypeScript Language Grammar
# Based on ECMAScript 2016 Language Grammar (Standard ECMA-262, 7th Edition / June 2016)
# Extended with TypeScript 2.8+ features (see typescriptlang.org)
# Optimized for static compilation to native binaries via LLVM
language ts(go);

lang = "ts"
package = "github.com/inspirer/textmapper/parsers/ts"
...
%s initial, template, templateDiv, templateExpr, templateExprDiv;
...
<initial, template, templateDiv, templateExpr, templateExprDiv> {
...
<initial, templateExpr, templateExprDiv>
'}': /\}/
	...
	if l.State <= StateTemplateExprDiv {
      ...
      token.IN, token.DO, token.RETURN, token.CASE, token.THROW, token.ELSE:
		...
		case token.SINGLELINECOMMENT, token.MULTILINECOMMENT:
			break
		case token.EXCL:
			// Always enable TypeScript non-null assertion
			switch l.token {
